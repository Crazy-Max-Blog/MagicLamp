...
void gh_sets(GH::Builder& b) {
  ...
  if(b.Input(sets.wifi.ssid).label("Ssid").click())      Serial.println((String)sets.wifi.ssid.ptr);
  if(b.Pass(sets.wifi.pass ).label("Password").click())  Serial.println((String)sets.wifi.pass.ptr);
  if(b.Button().label("Save and Restart").click())   {data.updateNow();}
  
  b.Label().valueStr("MQTT").noLabel(1).noTab(1);
  if(b.Input(sets.mqtt.host).label("Host").click())      Serial.println((String)sets.mqtt.host.ptr);
  if(b.Input(sets.mqtt.port).label("Port").click())      Serial.println((String)sets.mqtt.port.ptr);
  if(b.Input(sets.mqtt.ssid).label("Login").click())     Serial.println((String)sets.mqtt.ssid.ptr);
  if(b.Pass(sets.mqtt.pass).label("Password").click())   Serial.println((String)sets.mqtt.pass.ptr);
...
void mqtt_reconnect() {
  if(WiFi.status() == WL_CONNECTED) 
    hub.mqtt.config(sets.mqtt.host, sets.mqtt.port.toInt(), sets.mqtt.ssid, sets.mqtt.pass);  // + MQTT
}
