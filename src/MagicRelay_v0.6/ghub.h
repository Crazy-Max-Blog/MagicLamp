...
void gh_sets(GH::Builder& b) {
  ...
  if(b.Button().label("Save and Update").noTab(1).click())    {data.updateNow();pinMode(sets.devs.led.pin, OUTPUT);}
  
  b.Label().valueStr("Button").noLabel(1).noTab(1);
  b.Spinner(&sets.devs.btn.pin).label("Pin").noTab(1).click();                           //)      //Serial.println(sets.led.pin);
  if(b.Button().label("Save and Update").noTab(1).click())    {data.updateNow();btn=button(sets.devs.btn.pin);}
...
void ghub_begin() {    
  ...
  hub.onBuild(build);
  hub.setVersion((String)"Crazy-Max-Blog/MagicRelay@"+(String)PR_VERSION);
  hub.begin();
}
